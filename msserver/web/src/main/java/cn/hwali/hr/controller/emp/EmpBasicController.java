package cn.hwali.hr.controller.emp;

import cn.hwali.hr.model.*;
import cn.hwali.hr.service.*;
import cn.hwali.hr.utils.POIUtils;
...
//    public RespPageBean getEmployeeByPage(@RequestParam(defaultValue = "1") Integer page, @RequestParam(defaultValue = "10") Integer size, ) {
    ...
    @DeleteMapping("/")
    public RespBean deleteEmpsByIds(Integer[] ids){
        if (employeeService.deleteEmpsByIds(ids) == ids.length){
            return RespBean.ok("删除成功！");
        }
        return RespBean.error("删除失败！");
    }
    ...
    public RespBean maxWorkID() {
        RespBean respBean = RespBean.build().setStatus(200)
                .setObj(String.format("%08d", employeeService.maxWorkID() + 1)); //补齐 0 并对齐
        return respBean;
    }
